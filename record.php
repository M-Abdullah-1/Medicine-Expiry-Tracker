...
<body>
 <div class="bgPic">
    ...
    <div class="row py-3 height">
      <div class="col-sm-10 offset-sm-1">
        <div class="d-flex justify-content-between align-items-center my-1">
          <h1 class="text-dark">Selling Record</h1>
          <div class="d-flex">
            <form class="d-flex" id="search-record">
              <input id="searchByName"  class="form-control " type="text" placeholder="Search...">
              <input id="searchByDate" type="date" class="form-control ml-2" placeholder="Date....">
              <button id="searchByDateBtn" type="button" onclick="showByDate()" class="btn  text-light ml-0 mr-2">Apply</button>
              <button onclick="refresh()" class="btn bgOpacity mr-2 text-light"><i class="fas fa-sync-alt"></i></button>
            </form>
            <!-- <input  class="form-control mx-2" type="text" placeholder="Search By Name..."> -->
          </div>
        </div>
        <div id="listOfSellingRecord">
        ...
        <div id="sellingRecordSection"></div>
      </div>
    </div>
    <!-- table section end -->
  </div>
  ...
  $(document).ready(function() {
    ...
    //   'success'
    // )
      getListOfSellingRecord(0,1);
      putCurrentDateInSearchByDateBar();
      searchingRecord();
      notifyExpiryData(0);
      showExistingDateRecord();
  ...
  //
  function showByDate() {
    let searchingDate = $('#searchByDate');
    $.ajax({
      url: 'ajax.php',
      method: 'post',
      datatype: 'text',
      data: {
        key: 'showByDate',
        searchingDate : searchingDate.val()
      },
      success: function(response) {
        // alert(response);
        if (response != 'noMore') {
          $('#sellingRecordSection').empty();
          $('#sellingRecordSection').append(response);
        } else {
          // alert('no Record');
          // toastr["warning"]('No such Record');
          Swal.fire(
            'Error',
            'No Such Record',
            'error'
  ...
  // get list of selling record from db
  function getListOfSellingRecord(start,limit) {
    $.ajax({
      url: 'ajax.php',
      method: 'post',
      datatype: 'text',
      data: {
        key: 'getListOfSellingRecord',
        start: start,
        limit: limit
      },
      success: function(response) {
        ...
        if (response != 'noMore') {
          $('#sellingRecordSection').append(response);
          start += limit;
          getListOfSellingRecord(start,limit);
        }
      }
    });
  }
  // put current date in searchByDate bar
  function putCurrentDateInSearchByDateBar() {
    let d = new Date();
    ...
    let searchDateBar = $('#hiddenCurrentDate');
    searchDateBar.val(currentDate);
  }
  // sarching record function
  function searchingRecord() {

    const list = document.querySelector('#sellingRecordSection');

    const searchNameBar = document.forms['search-record'].querySelector('#searchByName');

    const searchDateBar = document.forms['search-record'].querySelector('#searchByDate');
    // search by name
    searchNameBar.addEventListener('keyup',function(e) {
      const term = e.target.value.toLowerCase();
      const tables = list.getElementsByClassName('singleTable');

      Array.from(tables).forEach(function(table) {
        // const buyerName = table.firstElementChild.textContent;
        const buyerName = table.getElementsByTagName('span')[0].textContent;
        const buyingDate = table.getElementsByTagName('span')[1].textContent;
        console.log(buyerName);
        if (buyerName.toLowerCase().indexOf(term) != -1) {
    ...
    // search by date
    // searchDateBar.addEventListener('keyup',function(e) {
    //   const term = e.target.value;
    //   const tables = list.getElementsByClassName('singleTable');
    //   console.log(tables);
    //   Array.from(tables).forEach(function(table) {
    //     const buyingDate = table.getElementsByTagName('span')[1].textContent;
    //     if (buyingDate.indexOf(term) != -1) {
    //       table.style.display = 'block';
    //     } else {
    //       table.style.display = 'none';
    //     }
  ...
  // show list of present date
  function showExistingDateRecord() {
    let d = new Date();
    let yearNumber = d.getFullYear();
    let monthNumber = d.getMonth()+1;
    let dateNumber = d.getDate();
  ...
  function saveEditData() {
    ...
    if (empty(product_name) && empty(exp_date) && empty(pack_size) && empty(quantity) && empty(price)) {
      $.ajax({
        ...
        success: function(response) {
          $("#country_"+rowID.val()).html(product_name.val());
          $("#tableManager").modal('hide');
          notifyExpiryData(0);
          // toastr notification
          toastr["success"]('Requested completed.');
